...
object Retry {
  ...
  def retry[T](maxRetry: Int,
               ...
               ignoreThrowable: Throwable => Boolean = noIgnore)(block: => T)(implicit ctx: ExecutionContext) = {
...
//object MyRetry {
//
//  def retry[T](timeout: Duration = 1 hour)(block: => Future[T]): Future[T] = {
//    block map {
//      case x => x
//    }
//  }
